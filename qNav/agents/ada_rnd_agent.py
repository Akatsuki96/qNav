...
#        blk_len = ceil(np.mean(blks)) if len(blks) > 0 else 0 
    ...
    def get_action(self, raw_obs, state, features, test, action, s_thr):
        used_mem = self.stat_memory[-self.curr_len:]
        #if test and len(used_mem[used_mem >= s_thr]) == 0:
    ...
    def _step(self, horizon = None, callback = None, clip_position=False, test = False):
        ...
        while horizon is None or k < horizon:
            ...
            path.append(action)
            if action == RND_ACTION:
                raw_obs, reward, terminated = self.env.step(self.rnd_state.randint(0, 4), clip_position = clip_position)
            else:
