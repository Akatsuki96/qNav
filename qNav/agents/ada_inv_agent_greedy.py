...
#        blk_len = ceil(np.mean(blks)) if len(blks) > 0 else 0 
        self.curr_len = self.memory #blk_len if blk_len > self.min_mem else self.min_mem #blk_len if blk_len > self.min_mem else self.min_mem

    def _set_clen(self):
        self.curr_len = self.last_blank if self.last_blank > self.min_mem else self.memory#min_mem
